...
def adopt_weight(weight, use_weight=False, value=0.0):
    if not use_weight:
        weight = value
    return weight
...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(
        ...
        kl_loss = (
            ...
            * torch.sum(
                1
                + posteriors.latent_dist.logvar
                - posteriors.latent_dist.mean.pow(2)
                - posteriors.latent_dist.logvar.exp()
            )
            / batch_size
        )
        ...
        if optimizer_idx == 0:
            ...
            disc_factor = adopt_weight(
                self.disc_factor, use_weight=self.discriminator_active
            )
            loss = (
                weighted_nll_loss
